...
Search and availability require no account. OpenTable and Tock attach booking use the signed-in session in the attached Chrome profile (Tock keeps the stealth-headless session fallback); Resy booking uses its saved API token.
