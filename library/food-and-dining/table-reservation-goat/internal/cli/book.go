...
type bookResult struct {
	...
	PageState            json.RawMessage `json:"page_state,omitempty"` // sanitized attach diagnostics; never includes URL queries
...
func bookOnOpenTable(ctx context.Context, session *auth.Session, slug, date, hhmm string, party int, dryRun bool, out bookResult) (bookResult, error) {
	...
	// mode may continue when the Surf/Akamai path is blocked because the fresh
	// browser target performs its own signed-in profile check before booking.
	...
	// No configured endpoint preserves the existing Surf/REST behavior below.
	if attachConfigured {
		...
		if chromeErr != nil {
			...
			if errors.As(chromeErr, &typedChromeErr) {
