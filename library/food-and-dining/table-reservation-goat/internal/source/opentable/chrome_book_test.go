...
func TestOpenTableChromeBookTypedErrors(t *testing.T) {
	typed := []error{ErrAttachUnreachable, ErrNotSignedIn, ErrSelectorDrift, ErrFormValidation}
...
func TestOpenTableFormValidationErrorNamesRedactedRequiredControls(t *testing.T) {
	...
	text := err.Error()
	if !strings.Contains(text, "I agree to this restaurant's terms") || !strings.Contains(text, "Account identity") {
...
func TestPageStateSnapshotRedactsAccountIdentity(t *testing.T) {
	...
	for _, want := range []string{"Account switch", "Account identity", "Complete reservation", "Agree to terms", "Personal contact"} {
